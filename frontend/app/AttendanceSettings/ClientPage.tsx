...
function createDefaultSettings(year: number): YearSettings {
  const gasolineByMonth = MONTHS.reduce(
    (acc, { m }) => {
      acc[m] = 0;
      return acc;
    },
...
export default function ClientPage({ user }: { user?: any } = {}) {
  ...
  const [draft, setDraft] = useState<YearSettings>(() =>
    createDefaultSettings(currentYear),
  );
  const [saved, setSaved] = useState<YearSettings>(() =>
    createDefaultSettings(currentYear),
  );
  ...
  const dirty = useMemo(() => {
    ...
    for (const { m } of MONTHS) {
      if ((draft.gasolineByMonth[m] || 0) !== (saved.gasolineByMonth[m] || 0))
        return true;
    }

    if (draft.vacationDates.size !== saved.vacationDates.size) return true;
    for (const k of draft.vacationDates)
      if (!saved.vacationDates.has(k)) return true;
